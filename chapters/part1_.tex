...
Рассмотрим сплошной цилиндр с осью симметрии \(oz\) и радиусом \(b\) с изменением радиальной температуры, такой что \(\theta(r)=T(r)-T_0\), где \(T_0\) --- начальная температура. Предположим также плоскюу деформацию \(\epsilon_{zz}=\epsilon_{rz}=\epsilon_{\phi z}=0\). Тогда напряженно-деформированное состояние запишется в виде:
\begin{equation}
	\label{eq:equation1}
\begin{split}
